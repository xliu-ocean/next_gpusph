...
MYFlatRoughTank_v1::MYFlatRoughTank_v1(GlobalData *_gdata) : Problem(_gdata)
{
	...
	// add DEM
	const string dem_file = get_option("dem", "preprocessing/flat_bottom/cobble_surface_with_flat_sand_v1_fac0.0.txt");
...
//		add_flags<ENABLE_PLANES>()
	...
	//	cout << "\nwall height: " << wall_height << "\n";
                ...
                //        //Point(m_origin + make_double3(0, ly, -box_thickness)),
                        Point(make_double3(0,0,0) + make_double3(h_length+slope_length+slope2_length-m_deltap+1.001398e-2,0, -box_thickness)),
                        box_thickness, ly+0.01,  lz+box_thickness);
		// close wall
		...
		wall = addBox(GT_FIXED_BOUNDARY, FT_BORDER,
                        ...
                        lx + 2*box_thickness, ly + 2*m_deltap + 0.01, box_thickness);
        }
	GeometryID fluid;
        float z = 0;
        int n = 0;
        while (z < H) {
                ...
                //     l = h_length-10;
                //}
		if (l>=h_length+slope_length+slope2_length-11*m_deltap) {
			l = h_length+slope_length+slope2_length-11*m_deltap;
		}
...
void
MYFlatRoughTank_v1::moving_bodies_callback(const uint index, Object* object, const double t0, const double t1,
